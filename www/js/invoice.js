(function () {
	const invoicer = angular.module('invoicer', ['xeditable', 'ui.sortable', 'ui.router']);

	invoicer.run(editableOptions => {
		editableOptions.theme = 'bs3';
	});

	invoicer.config(($stateProvider, $urlRouterProvider) => {
	...
	invoicer.controller('invController', $scope => {
		$scope.date = new Date();
		...
		$scope.getTotal = function () {
			let total = 0;
			for (let i = 0; i < $scope.invoice.line_items.length; i++) {
				const line = $scope.invoice.line_items[i];
